<div>
	...
	if (isset($album->info)){
		...
		$tab = json_decode($album->getInfo());
		?>
		<div itemscope itemtype="http://schema.org/MusicAlbum">
			<h2 style="text-align:left" itemprop="name"><a title="Retour" class="cursor" onclick="backAlbum();" ><i
							class="fa fa-step-backward mycolor"></i></a>&nbsp;<?php echo $album->getName();?></h2>
			<span itemprop="byArtist" class="inv">{{ $album->artist->getName()}}</span>
				...
				foreach ($tab->album->tracks->track as $track){
					...
					<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
						...
						<div itemscope itemtype="http://schema.org/MusicRecording">
							<a class="cursor" onclick="$('#title').val('');$('#q').val('{{ str_replace("'","\\'",$album->artist->getName())}}');$('#album').val('{{ str_replace("'","\\'",$album->getName())}}');$('#title').val('{{ str_replace("'","\\'",$track->name)}}');searchX();">
