...
class SonosPHPController
{
	...
	*/
	...
	protected function Filter($subject,$pattern)
	{
        ...
        if (preg_match('/\<'.$pattern_escaped.'\>(.+)\<\/'.$pattern.'\>/s', $subject, $matches)) {
            return $matches[1];
        }
        return '';
	}
	...
	*/
	public function GetPositionInfo()
	{
		...
		$data["Title"] = html_entity_decode($this->Filter($xml,"dc:title"), ENT_QUOTES | ENT_HTML5, 'UTF-8');	// Track Title
		$data["AlbumArtist"] = html_entity_decode($this->Filter($xml,"r:albumArtist"), ENT_QUOTES | ENT_HTML5, 'UTF-8');		// Album Artist
		$data["Album"] = html_entity_decode($this->Filter($xml,"upnp:album"), ENT_QUOTES | ENT_HTML5, 'UTF-8');		// Album Title
		$data["TitleArtist"] = html_entity_decode($this->Filter($xml,"dc:creator"), ENT_QUOTES | ENT_HTML5, 'UTF-8');	// Track Artist
